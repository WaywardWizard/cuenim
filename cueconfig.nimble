...
# Nimble (why?) alters the location of .nimble file relative to src on installation
# breaking tasks.
let dthis = thisDir()
proc srcfolder(): string =
  if fileExists(dthis & "/src/cueconfig.nim"):
    "src"
  elif fileExists(dthis & "/cueconfig.nim"):
    dthis
  else:
    quit("Could not locate source folder for tasks")
const DOCFOLDER = "docs"

import std/[strformat,sequtils,sugar,cmdline,strutils]
task docgen, "Generate documentation":
  var cmdparams = @[""] #commandLineParams().join(" ").split(" docgen ")[1].split(" ")
  var docfolder = DOCFOLDER
  if cmdparams[0] != "" and fileExists(cmdparams[0]):
    echo "Documentation writing to folder cmdparams[0]"
    docfolder = cmdparams[0]
    echo "Extra docgen args: " & cmdparams
  ...
  subcmd = "doc"
  var target = srcfolder() & "/cueconfig.nim"
